...
- 카드 아래 버튼은 자주 누르는 **해결**·**또 하기**가 크고, **수정**·**삭제**는 아이콘만 있습니다
...
구간이 도는 동안 경과 시간은 폰을 바닥에 두고 떨어져서도 읽히게 크게 보여줍니다. 기술 연습 중에는 그 아래에 오늘 할 것의 cue가 뜨고, 웜업 구간에서는 웜업 러너를 바로 열 수 있습니다. 다른 탭에 있어도 타이머 탭 이름("운동 중", "휴식 중", "일시정지")으로 상태를 알 수 있습니다.
...
- 주 단위 추이 (쉰 주도 자리를 차지합니다. 7일 기간에는 숨깁니다)
- 운동한 날 캘린더 — 운동한 날을 누르면 그날 기록 목록으로 갑니다
- 연속 운동 일수 (고른 기간과 상관없이 전체 기록으로 셉니다)
...
시작을 누르면 항목을 한 장씩 넘기며 따라갈 수 있고(이전으로 돌아갈 수 있고, 중간에 닫았다 열면 보던 자리부터 이어집니다), 건너뛴 항목이 있어도 끝까지 진행했다면 그날 웜업은 완료로 봅니다. 완료 표시는 진행률을 100%로 부풀리지 않고 "2개 완료 · 7개 건너뜀"처럼 실제 숫자를 함께 적습니다.
...
- 직전 시즌과의 변화는 "5회 → 3회 · 개선"처럼 값과 말로 보여줍니다.
- 기록이 있는 종목은 단위를 바꿀 수 없습니다. 기록은 숫자만 저장하므로 단위가 바뀌면 지난 값의 뜻이 달라집니다.
